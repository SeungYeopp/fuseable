...
public class ProjectController {
    ...
    @DeleteMapping("")
    public ProjectDeleteDto deleteProject (Long projectId, @AuthenticationPrincipal UserDetailsImpl userDetails){ //프로젝트 Id + 로그인된 사용자
        return projectService.deleteProject(projectId, userDetails.getUser());
    }

    //프로젝트 선택
    @GetMapping("/{projectId}")
    public ProjectSelectDto selectProject (@PathVariable ("projectId") Long projectId, @AuthenticationPrincipal UserDetailsImpl userDetails){
        return projectService.selectProject(projectId, userDetails.getUser());
    }

    @PostMapping("/{projectId}")
    //front로부터 title명 요구
    public ProjectUpdateDto updateProject(@PathVariable ("projectId") Long projectId, @RequestBody ProjectTitleDto TitleDto, @AuthenticationPrincipal UserDetailsImpl userDetails) { //제목 + 로그인된 사용자
        return projectService.updateProject(projectId, TitleDto, userDetails.getUser());
    }
